            ...
            <div class="auth-fluid-form-box">
                <div class="card-body d-flex flex-column ">
                    ...
                    <div class="">
                        ...
                        <!-- form -->
                        <form action="{{ route('lms.login') }}" method="POST" id="adminLoginForm">
                            @csrf
                            <div class="mb-3">
                ...
                $('#adminLoginForm').on('submit', function(e) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            } else if (xhr.status === 403 && xhr.responseJSON?.csrf) {
                                $('input[name="_token"]').val(xhr.responseJSON.csrf);
                                $('#loginError').html(`<div class="text-danger">${xhr.responseJSON.message}</div>`);

                            } else if ((xhr.status === 401 || xhr.status === 403) && xhr.responseJSON?.message) {
                                $('#loginError').html(`<div class="text-danger">${xhr.responseJSON.message}</div>`);

                            } else if (xhr.status === 419) {
                                $('#loginError').html('<div class="text-danger">Your session has expired. Please refresh the page and try again.</div>');
