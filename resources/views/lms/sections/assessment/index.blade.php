    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                @if(auth()->user()->hasPermission('assessments.create'))
                    <div class="mb-3">
                        <a href="{{ route('lms.assessment.create') }}" class="btn btn-danger">
                ...
                <div class="modal fade" id="edit-exam-modal{{ $exam->id }}" tabindex="-1" role="dialog">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('lms.assessment.update', $exam->id) }}">
                                @csrf
                                ...
                                <div class="modal-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select name="subject_session_id" class="form-select" required>
                                                <option value="">-- Select Session --</option>
                                                @foreach($sessions as $session)
                                                    <option value="{{ $session->id }}" {{ $exam->subject_session_id == $session->id ? 'selected' : '' }}>
                                                        {{ $session->title }}
                                                    </option>
                                                @endforeach
                                            </select>
...
<script>
    ...
    $('.confirm-delete-exam').off('click').on('click', function () {
        let id = $(this).data('id');
        let url = '{{ route("lms.assessment.delete", ":id") }}'.replace(':id', id);
    ...
    $(document).ready(function () {
         let table = $('#exams-table').DataTable({
            serverSide: true,
            ajax: "{{ route('lms.assessment.ajaxList') }}",
            pageLength: 25,
            columns: [
                { data: 'name', name: 'name' },  
                  ...
                  $('#exams-table tbody').off('click', 'tr').on('click', 'tr', function (e) {
                    ...
                    if (!$(e.target).closest('td').is(':last-child')) {
                        ...
                        if (rowData && rowData.id) {
                            window.location.href = "{{ route('lms.assessment.questions', ':id') }}"
