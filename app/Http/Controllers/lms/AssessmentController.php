...
class AssessmentController extends Controller
{
    ...
    {
        $exams = Exam::where('type','Assessment')->get();
        $subjects = Subject::where('type', 'lms')->get();
        $batches = Batch::where('status', '1')->get();
        $sessions = SubjectSession::all();
        return view('lms.sections.assessment.index',compact('sessions','exams','subjects','batches'));
    }
    ...
    public function create()
    {
        ...
        $batches = Batch::where('status', '1')->get();
        $sessions = SubjectSession::all();
        return view('lms.sections.assessment.add',compact('sessions','subjects','batches'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::transaction(function () use ($validated) {
                ...
                $exam->subject_session_id    = $validated['subject_session_id'];
            ...
            return redirect()
                ->route('lms.assessment.index')
                ->with('message', 'Assessment created successfully.');

        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->back()->withErrors($e->errors())->withInput();
        } catch (\Exception $e) {
            Log::error('Assessment store failed', ['error' => $e->getMessage()]);
    ...
    public function ajaxList(Request $request)
    {
        ...
        return DataTables::of($exams)
            ...
            ->addColumn('action', function ($exam) {
                $actions = '';
                if (auth()->user()->hasPermission('assessments.update')) {
                ...
                if (auth()->user()->hasPermission('assessments.delete')) {
    ...
    public function update(Request $request, string $id)
    {
        ...
        $validated['type'] = 'Assessment';
        
        $exam->update($validated);

        return redirect()->route('lms.assessment.index')->with('message', 'Exam updated successfully.');
    }
    ...
    public function showQuestionsPage(Exam $exam)
    {
        ...
        return view('lms.sections.assessment.question_add', compact('exam'));
    }
    ...
    public function storeQuestion(Request $request, Exam $exam)
    {
        ...
        return redirect()->route('lms.assessment.questions', $exam->id)
